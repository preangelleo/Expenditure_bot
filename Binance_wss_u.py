...
def on_message(ws, message):
    ...
    if data['e'] == 'ORDER_TRADE_UPDATE' and data['o']['X'] == 'FILLED':
        ...
        if data['o']['S'] == 'BUY' and data['o']['ps'] == 'SHORT':
            try:
                df = pd.DataFrame(engine.connect().execute(text('SELECT * FROM umfuture_orders_short WHERE coin = :coin AND type = :type ORDER BY trade_time DESC LIMIT 1'), {'coin': data['o']['s'][:-4], 'type': 'SELL'}).fetchall())
                if not df.empty:
                    coin = data['o']['s'][:-4]
        ...
        if data['o']['S'] == 'SELL' and data['o']['ps'] == 'LONG':
            try:
                df = pd.DataFrame(engine.connect().execute(text('SELECT * FROM umfuture_orders_long WHERE coin = :coin AND type = :type ORDER BY trade_time DESC LIMIT 1'), {'coin': data['o']['s'][:-4], 'type': 'BUY'}).fetchall())
                if not df.empty:
                    coin = data['o']['s'][:-4]
