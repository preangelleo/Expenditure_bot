...
def get_resistant_prices(symbol, interval = '1w'):
    ...
    if not df.empty: 
        ...
        if current_price > 0:
            ...
            resistance_support_levels = [f"{format_number(price)}({format_number(100 * float(price) / current_price)}%)" for price in resistance_support_levels if price >= current_price * 1.2]
