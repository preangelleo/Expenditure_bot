...
def get_candlestick_data(symbol, interval, limit=1000, add_rsi=True):
    PATH = '/api/v3/klines'
    timestamp = int(time.time() * 1000)
    params = {
        'symbol': symbol,
    ...
    if r.status_code == 200:
        ...
        df = pd.DataFrame(data, columns=['OpenTime', 'Open', 'High', 'Low', 'Close', 'Volume', 'CloseTime', 'QuoteAssetVolume', 'TradeNum', 'TakerBuyBaseAssetVolume', 'TakerBuyQuoteAssetVolume', 'Ignore'])
        ...
        df['TradeNum'] = df['TradeNum'].astype(int)
        df['TakerBuyBaseAssetVolume'] = df['TakerBuyBaseAssetVolume'].astype(float)
        df['TakerBuyQuoteAssetVolume'] = df['TakerBuyQuoteAssetVolume'].astype(float)
        df['Ignore'] = df['Ignore'].astype(float)
        # if add_rsi: df['RSI'] = talib.RSI(df['Close'], timeperiod=14)
        return df

# Get BTC weekly candlestick data with RSI indicator from binance and plot it, save to file under net_profit_daily_record and send to chat_id
def plot_btc_weekly_rsi(chat_id=TG_BOT_OWNER_ID):
    filename = f"net_profit_daily_record/BTC_weekly_rsi_{datetime.now().strftime('%Y-%m-%d')}.png"
    # check if the file exists, if yes, return the file name
    if os.path.isfile(filename): return send_img(chat_id, filename)

    # get BTC weekly candlestick data with RSI indicator
    df = get_candlestick_data('BTCUSDT', '1w', 1000, False)
    if df.empty: return send_img(chat_id, f"net_profit_daily_record/Leowang.net.jpg")

    # Plotting
    plt.figure(figsize=(10, 6))
    plt.plot(df['OpenTime'], df['RSI'], marker='o')
    plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
    plt.gca().xaxis.set_major_locator(mdates.DayLocator())
    plt.xticks(rotation=45)
    plt.xlabel('Date')
    plt.ylabel('RSI')
    plt.title('BTC Weekly RSI')
    plt.grid(True)
    # plt.show()
