...
def get_resistant_prices(symbol, interval = '1w'):
    ...
    if not df.empty: 
        ...
        if current_price > 0:
            ...
            resistance_support_levels = [f"{format_number(price)}({format_number(100 * (float(price) - current_price) / current_price)}%)" for price in resistance_support_levels if price >= current_price * 1.2]
            # MATIC: Current Price 1.13, Target  1.56 / 1.74 / 2.1 / 2.45 / 2.9
            reply_msg = f"{coin}: Current Price {format_number(current_price)}, Resistant prices: {', '.join(resistance_support_levels[0:3])}"
            return reply_msg
    return
...
def get_support_prices(symbol, interval = '1w'):
    ...
    if not df.empty: 
        ...
        if current_price > 0:
            resistance_support_levels = calculate_resistance_support(df)
            resistance_support_levels = [f"{format_number(price)}({format_number(100 * (float(price) - current_price) / current_price)}%)" for price in resistance_support_levels if price <= current_price * 0.9]
